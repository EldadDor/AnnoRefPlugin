...
import com.idi.intellij.plugin.query.sqlref.action.AnnoRefCreateClassDialog;
...
public class CreateAnnoRefClassForUnusedTagFix extends LocalQuickFixBase {
	...
	public AnnoRefCreateClassDialog createAnnoRefClass(Project project) {
		...
		final String[] parentInterfaceName = SQLRefConfigSettings.getInstance(project).getSqlRefState().ANNO_REF_SUPER_INTERFACE.split("\\.");
...
//		final PsiAnnotation importClass1 = JavaPsiFacade.getInstance(project).getElementFactory().createAnnotationFromText("@" + SQLRefConfigSettings.getInstance(project).getSqlRefState().ANNOREF_ANNOTATION_FQN, null);
		final PsiClass importClass2 = psiElementFactory.createClass(ClassUtil.extractClassName(SQLRefConfigSettings.getInstance(project).getSqlRefState().ANNO_REF_SUPER_INTERFACE));
		final PsiJavaCodeReferenceElement referenceElementByFQClassName = psiElementFactory.
				createReferenceElementByFQClassName(SQLRefConfigSettings.getInstance(project).
						getSqlRefState().ANNO_REF_SUPER_INTERFACE, GlobalSearchScope.projectScope(project));
