...
public class ClassScanningTask extends IDIAbstractTask {
	...
	public void runTask() {
		...
		} catch (IDIProcessCancelledException processCancelled) {
...
//		PsiFile psiFile1 = PsiManager.getInstance(project).findFile(classFile);
		...
		if (AnnoRefConfigSettings.getInstance(project).getAnnoRefState().ENABLE_UTIL_CLASS_SCAN) {
			scanForRequiredMethodCalls(psiFile, classFile);
		}
	}
	 ...
	 * @param classFile
	 */
	private void scanForRequiredMethodCalls(PsiElement psiFile, VirtualFile classFile) {
		if (psiFile instanceof PsiJavaFile) {
			for (final PsiImportStatement psiImportStatement : ((PsiJavaFile) psiFile).getImportList().getImportStatements()) {
				if (AnnoRefConfigSettings.getInstance(project).getAnnoRefState().ANNOREF_UTIL_CLASS_FQN.equals(psiImportStatement.getQualifiedName())) {
					if (logger.isDebugEnabled()) {
						logger.debug("scanClassFile(): psiImportStatement.getQualifiedName()=" + psiImportStatement.getQualifiedName());
					}
					final FilterElementProcessor processor = new FilterElementProcessor(new StringLiteralElementFilter(project, classFile));
					...
					/*	final String refId = StringUtils.cleanQuote(((PsiCall) psiElement).getArgumentList().getExpressions()[0].getText());
						...
						refRepository.addUtilClassMethodCallInformationToRepository(refId, classFile, psiElement);*/
...
//							final SQLRefReference refReference = refRepository.getSQLRefReferenceForID(refId);
