...
import com.idi.intellij.plugin.query.annoref.common.ClassAnnoRefSearchStateEnum;
...
public class SQLRefDataAccessor implements ProjectComponent {
	...
	public Boolean isMatchFileName(String fileName) {
			...
			case 1:
				if (editor != null) {
					...
					if (xmlToClassPair != null && xmlToClassPair.getSecond()) {
						...
						com.idi.intellij.plugin.query.annoref.repo.model.SQLRefReference sqlRefReferenceForID = ServiceManager.getService(psiFile.getProject(), SQLRefRepository.class).getSQLRefReferenceForID(sqlRefId);
